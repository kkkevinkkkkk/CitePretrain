from utils import DATASET_PROFILES, make_demo, make_demo_messages, OPENAI_MODELS
from transformers import AutoTokenizer
class Prompter:
    def __init__(self, model_name="gpt-3.5-turbo",
                 dataset_name=None,
                 n_shot=0,
                 n_doc=0,
                 n_doc_in_demo=0,
                 fewer_doc_in_demo=False,
                 no_doc_in_demo=True,
                 use_shorter="text",
                 oracle_doc=False,
                 demo_prompt_idx=None,
                 ):

        self.model_name = model_name
        self.dataset_name = dataset_name
        self.n_shot = n_shot
        self.n_doc = n_doc
        self.n_doc_in_demo = n_doc_in_demo
        self.fewer_doc_in_demo = fewer_doc_in_demo
        self.no_doc_in_demo = no_doc_in_demo
        self.use_shorter = use_shorter
        self.oracle_doc = oracle_doc
        self.demo_prompt_idx = demo_prompt_idx
        if model_name.startswith("gpt") or model_name in ["meta-llama/Meta-Llama-3-8B"] or model_name in OPENAI_MODELS:
        # if model_name.startswith("gpt"):
            self.tokenizer = None
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(model_name)

        self.head_prompt = None
        # if the tokenizer is in base models list, we don't apply the chat template
        self.base_models_list = ["Qwen/Qwen2.5-7B", "Qwen/Qwen2.5-14B"]

    def generate_main_task_input(self, eval_item=None,
                                 faithful_type=None, citation_type=None, **kwargs):
        if self.dataset_name == "quality":
            return self.generate_quality_input(eval_item)

        prompt_data = DATASET_PROFILES[self.dataset_name]
        instruction_key = "instruction" if faithful_type is None else f"instruction_{faithful_type}_faithful"
        demos_key = f"{faithful_type}_faithful_demos" if faithful_type == "cot_situated" else "demos"
        if citation_type is not None:
            instruction_key = "instruction_citation"
            if "ic2" in citation_type:
                instruction_key = "instruction_citation2"
            if "st" in citation_type:
                demos_key = "demos_citation_st"
            elif "refusal" in citation_type:
                demos_key = "demos_citation_rag_refusal"
                instruction_key = "instruction_citation_rag_refusal"
            elif "rag2" in citation_type:
                demos_key = "demos_citation_rag"
                instruction_key = "instruction_citation_rag"
            elif "rag" in citation_type:
                demos_key = "demos_citation_rag"
            else:
                demos_key = "demos_citation"


        demo_prompt_key = f"demo_prompt_{self.demo_prompt_idx}" if self.demo_prompt_idx else "demo_prompt"

        messages = make_demo_messages(prompt_data,
                                      n_shot=self.n_shot,
                                      n_doc=self.n_doc,
                                      n_doc_in_demo=self.n_doc_in_demo,
                                      fewer_doc_in_demo=self.fewer_doc_in_demo,
                                      no_doc_in_demo=self.no_doc_in_demo,
                                      use_shorter=self.use_shorter,
                                      instruction_key=instruction_key,
                                      demos_key=demos_key,
                                      demo_prompt_key=demo_prompt_key)



        test_input = make_demo(eval_item, prompt_data[demo_prompt_key],
                               doc_prompt=prompt_data["doc_prompt"],
                               instruction=prompt_data[instruction_key],
                               n_doc=self.n_doc,
                               test=True)
        if citation_type and "answer" in citation_type:
            answer = eval_item["Answer"]["Value"]
            test_input += answer + "\n\n"

        messages.extend([{"input": test_input}])

        return messages

    def generate_quality_input(self, eval_item):
        prompt_data = DATASET_PROFILES[self.dataset_name]
        few_shot_prefix = prompt_data["few_shot_prefix"]
        text_input = "### Question\n" + eval_item["question"] + "\n### Choices\n" + eval_item["choices"]
        messages = [{"input": few_shot_prefix + text_input}]
        return messages



    def process_messages(self, messages):
        if self.tokenizer and self.tokenizer.chat_template and self.tokenizer.name_or_path not in self.base_models_list:
            new_messages = []
            for message in messages:
                new_message = []
                if "input" in message:
                    new_message.append({"role": "user", "content": message["input"]})
                if "output" in message:
                    new_message.append({"role": "assistant", "content": message["output"]})
                new_messages.extend(new_message)
            text_input = self.tokenizer.apply_chat_template(new_messages, tokenize=False, add_generation_prompt=True)
        elif self.tokenizer:
            text_input = ""
            for message in messages:
                if "input" in message:
                    text_input += message["input"]
                if "output" in message:
                    text_input += message["output"] + self.tokenizer.eos_token + "\n\n"
        else:
            text_input = ""
            for message in messages:
                if "input" in message:
                    text_input += message["input"]
                if "output" in message:
                    text_input += message["output"] + "\n\n"
        return text_input



    def generate_text_input(self, task_type="main", process_message=True, **kwargs):
        citation_type = None
        if task_type == "citation":
            citation_type = "ctype-0"
        elif task_type == "citation_st":
            citation_type = "st"
        elif task_type == "citation_answer":
            citation_type = "answer"
        elif task_type == "citation_rag":
            citation_type = "rag"
        elif task_type == "citation_rag2":
            citation_type = "rag2"
        elif task_type == "citation_rag_refusal":
            citation_type = "rag_refusal"
        elif task_type == "citation_ic2":
            citation_type = "ic2"
        elif task_type == "citation_st_ic2":
            citation_type = "st_ic2"

        messages = self.generate_main_task_input(eval_item=kwargs["eval_item"],
                                                 faithful_type=kwargs.get("faithful_type"),
                                                 citation_type=citation_type)

        if process_message:
            text_input = self.process_messages(messages)
        else:
            text_input = messages

        return text_input