...
def evaluate_with_llm(pipe, question, correct_answers, response):
    temperature = 0.0
    max_tokens = 256
    ...
    while not is_valid_eval and current_attempt < max_eval_attempts:
        outputs = pipe(fresh_eval, temperature=temperature, max_tokens=max_tokens)
        result = outputs[0]['generated_text']
        is_valid_eval, eval_result = extract_ratings(result)
        current_attempt += 1

    is_correct = eval_result['rating'] == 'TRUE'
